...
void VnRos::connect() {
    ...
    for (const auto& baudrate : vnSensor.supportedBaudrates()) {
        baudrateQueue.push_back(baudrate);
    }
...
void VnRos::pubOdom(vn::sensors::CompositeData& cd) {
    ...
    if (cd.hasPositionEstimatedEcef() && (cd.positionEstimatedEcef() != vn::math::vec3d{0.0})) {
        ...
        // print copyable data for new ref position
        ROS_INFO_STREAM(std::setprecision(15)
                        << std::fixed << "\nvn::math::vec3d dragvollPos = {" << posEcef[0] << ", " << posEcef[1]
                        << ", " << posEcef[2] << "};\nconstexpr double phi = " << lla[0] << " * M_PI / 180.0;"
                        << "\nconstexpr double lambda = " << lla[1] << " * M_PI / 180.0;");
    }
...
void VnRos::writeSettings() {
    ...
    if (baseline.uncertainty[0] > maxUncertainty ||
        baseline.uncertainty[1] > maxUncertainty ||
        baseline.uncertainty[2] > maxUncertainty) {
        ROS_WARN_STREAM("Baseline uncertainty is larger than 2.5% of baseline lenght. This means that the INS will never reach mode 2");
    }
    // write baseline.
